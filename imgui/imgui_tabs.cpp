...
{
	ImGuiContext& g = *GImGui;
	const float rounding = 0.5f;// ImMin(g.FontSize * 0.35f, bb.GetWidth() * 0.5f);
...
void ImGui::StyleColorsMaze()
{
	...
	colors[ImGuiCol_WindowBg] = ImVec4(0.26f, 0.26f, 0.26f, 1.0f);
	...
	colors[ImGuiCol_ResizeGripHovered] = ImVec4(0.59f, 0.59f, 0.59f, 1.00f);
...
void ImGui::SexyCheckBox(char *checkboxtext, int index, bool &controller, ImVec4 &colorcontroller, ImVec4 &colorcontroller2)
{
	ImGui::ColorButton(string("colorbutton##" + to_string(index)).c_str(), colorcontroller, ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_NoInputs);
	ImGui::SameLine();

	if (ImGui::BeginPopupContextItem(string("colorbuttonrexy##" + to_string(index)).c_str(), 0))
	{
		ImGui::ColorPicker3(string("ColorPicker##" + to_string(index)).c_str(), (float*)&colorcontroller, ImGuiColorEditFlags_PickerHueWheel | ImGuiColorEditFlags_NoLabel  | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoInputs);
		if (ImGui::Button("X"))
			ImGui::CloseCurrentPopup();
		ImGui::EndPopup();
	}
	ImGui::ColorButton(string("colorbutton2##" + to_string(index)).c_str(),colorcontroller2, ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_NoInputs);
	ImGui::SameLine();

	if (ImGui::BeginPopupContextItem(string("colorbuttonrexy2##" + to_string(index)).c_str(), 0))
	{

		ImGui::ColorPicker3(string("ColorPicker2##" + to_string(index)).c_str(), (float*)&colorcontroller2, ImGuiColorEditFlags_PickerHueWheel | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoInputs);
		if (ImGui::Button("X"))
			ImGui::CloseCurrentPopup();
		ImGui::EndPopup();
	}
	...
	ImGui::SameLine();
	ImGui::Checkbox(checkboxtext, &controller);
}
int current_tab = -1;
int ImGui::TabButtonGIndex()
{
	return current_tab;
}
bool ImGui::TabButton(char* tabname, int index)
{
	ImGui::NewLine(); ImGui::SameLine(25); if (ImGui::TabButtonEx(tabname, ImVec2(100, 50), 0, index == current_tab ? true : false))
	{
		current_tab = index;
	}
	return index == current_tab ? true : false;
}
bool ImGui::TabButtonEx(const char* label, const ImVec2& size_arg, int flags,bool selected)
{
	...
	// Render
	const ImU32 col = GetColorU32((hovered && held ) ||(selected) ? ImGuiCol_ButtonActive :  hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
